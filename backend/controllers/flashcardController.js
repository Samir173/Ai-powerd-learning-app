...
// @access Private
export const markFlashcardAsReviewed = async (req, res, next) => {
  try {
...
// @desc  toggle star/unstar flashcard
...
export const toggleStarFlashcard = async (req, res, next) => {
  try {
    ...
    if (!flashcardSet) {
      return res.status(404).json({
        success: false,
        error: "Flashcard set or car not found",
        statusCode: 404,
      });
    }
    const cardIndex = flashcardSet.cards.findIndex(
...
// @route DELETE/api/flashcards/:cardId/review
// @access Private
